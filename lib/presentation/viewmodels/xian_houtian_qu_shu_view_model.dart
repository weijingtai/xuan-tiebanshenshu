...
class XianHoutianQuShuViewModel extends BaseTiaoWenListViewModel {
  ...
  /// Domain层结果（包含XianHoutianQuShuBaseNumberModel）
  MultiBaseNumberResult? _domainResult;

  XianHoutianQuShuViewModel(this._useCase);

  @override
  String get name => '先后天卦取数ViewModel';
  ...
  Future<void> calculateTiaoWenList() async {
    ...
    await safeExecute(() async {
      ...
      // 保存domain结果以便访问XianHoutianQuShuBaseNumberModel
  ...
  /// 获取XianHoutianQuShuBaseNumberModel（完整的中间结果）
  XianHoutianQuShuBaseNumberModel? get xianHoutianQuShuModel {
    if (!hasResult || _domainResult == null) return null;

    // 从sourceData中获取XianHoutianQuShuBaseNumberModel
    final sourceData = _domainResult!.sourceData;
    if (sourceData.containsKey('xianHoutianQuShuBaseNumberModel')) {
      return sourceData['xianHoutianQuShuBaseNumberModel']
          as XianHoutianQuShuBaseNumberModel?;
    }

    return null;
  }

  /// 是否有XianHoutianQuShuModel
  bool get hasXianHoutianQuShuModel => xianHoutianQuShuModel != null;
  ...
  String get xiantianGuaDisplayText {
    final model = xianHoutianQuShuModel;
