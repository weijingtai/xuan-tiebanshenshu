...
/// 先后天卦取数结果展示卡片
///
/// 展示先后天卦取数法的完整计算结果和条文信息
/// 基于六爻纳甲配置，使用±48×倍数[2,4,8,16]进行条文扩展
class XianHoutianQuShuCard extends StatefulWidget {
  final XianHoutianQuShuViewModel viewModel;
  final bool initiallyExpanded;

  const XianHoutianQuShuCard({
    super.key,
  ...
  @override
  State<XianHoutianQuShuCard> createState() => _XianHoutianQuShuCardState();
}

class _XianHoutianQuShuCardState extends State<XianHoutianQuShuCard> {
  ...
  Widget _buildGuaSummary(ThemeData theme) {
    final model = widget.viewModel.xianHoutianQuShuModel;
    if (model == null) {
      return Container(
      ...
      children: [
        Padding(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // 先天卦条文扩展（±48×倍数[2,4,8,16]）
              _buildTiaoWenExpansionCard(
                theme,
                '先天卦条文扩展（±48×倍数[2,4,8,16]）',
                '基础数 ± 48×[2, 4, 8, 16] = ±[96, 192, 384, 768]',
                widget.viewModel.xiantianTiaoWenNumbers,
                theme.colorScheme.primary,
              ),
              const SizedBox(height: 12.0),
              // 后天卦条文扩展（±48×倍数[2,4,8,16]）
              _buildTiaoWenExpansionCard(
                theme,
                '后天卦条文扩展（±48×倍数[2,4,8,16]）',
                '基础数 ± 48×[2, 4, 8, 16] = ±[96, 192, 384, 768]',
