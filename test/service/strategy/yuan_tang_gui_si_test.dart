...
void main() {
  ...
  group('步骤3：元堂装卦 - 癸巳甲子丁酉癸卯', () {
    test('应该判断卯时为阳时', () {
      ...
      expect(model.timeYinYang, equals(YinYang.YANG), reason: '卯时属于阳时（子丑寅卯辰巳）');
    });
  ...
  group('步骤4：生成后天卦 - 癸巳甲子丁酉癸卯', () {
    test('后天卦应该是坎震（水雷屯）', () {
      ...
      expect(model.houtianGua, equals(Enum64Gua.shui_lei_tun), reason: '二爻爻变且上下卦互换后应该得到坎震（水雷屯）');
    });
  ...
  group('完整计算流程验证 - 癸巳甲子丁酉癸卯', () {
    ...
    test('所有关键字段应该已填充', () {
      ...
      expect(model.houtianGua, equals(Enum64Gua.shui_lei_tun), reason: '后天卦应该是坎震');
      expect(model.xiantianGuaHu, isNotNull, reason: '先天卦互卦应该已计算');
      expect(model.houtianGuaHu, isNotNull, reason: '后天卦互卦应该已计算');
    });
